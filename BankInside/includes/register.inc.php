...
if (isset($_POST['username'], $_POST['email'], $_POST['p'])) {
	...
	//
	$prep_stmt = "SELECT id_user FROM bnk_user WHERE email = ? LIMIT 1";
	...
	if ($stmt) {
		...
		if ($stmt->num_rows == 1) {
			...
			$error_msg .= '<p class="error">Usuário com este e-mail já foi cadastrado anteriormente!</p>';
		}
	} else {
	...
	if (empty($error_msg)) {
		...
		if ($insert_stmt = $mysqli->prepare("INSERT INTO bnk_user (username, email, password, salt) VALUES (?, ?, ?, ?)")) {
